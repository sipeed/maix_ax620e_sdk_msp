...
)
{
    ...
    for (int i = 0; i < AX_AF_STAT_NUM; i++)
    {
        pAfInitPara->tAfMode.nfv_mode[i] = pAfStatParam->tAfMode.nfv_mode[i];
    }
    ...
    for (int i = 0; i < AX_AF_STAT_NUM; i++)
    {
        pAfInitPara->tAfCoring.nCoringPixSumflg[i] = pAfStatParam->tAfCoring.nCoringPixSumflg[i];
    }
...
void *AfRun(void *args)
{
    ...
    while (!gLoopExit) {
        if (gCams[i].bOpen) {
            axRet = AX_ISP_GetIrqTimeOut(gCams[i].nPipeId, AX_IRQ_TYPE_AF_DONE, 200);
            if (axRet == 0) {
