...
#define SKEL_SAMPLE_OUTPUT_PET_PATH "pet"
#define SKEL_SAMPLE_OUTPUT_GESTURE_PATH "gesture"
#define SKEL_SAMPLE_OUTPUT_PACKAGE_PATH "package"
#define SKEL_SAMPLE_OUTPUT_EBIKE_PATH "ebike"
...
static AX_VOID ShowUsage(AX_VOID) {
    ...
    printf("-F, \tFramerate control for yuv video(src:dst)(default=25:25)\n");
    printf("-Q, \tPush quality threshold((float: 0-1), default=0)\n");
    printf("-P, \tPose picth threshold(face push)((float: 0-180), default=180)\n");
    printf("-Y, \tPose yaw threshold(face push)((float: 0-180), default=180)\n");
    printf("-R, \tPose roll threshold(face push)((float: 0-180), default=180)\n");
    printf("-B, \tBlur threshold(face push)((float: 0-1), default=1)\n");
    printf("-f, \tFace feature analyzer enable((unsigned int), default=0)\n");
    printf("-c, \tConfidence((float: 0-1), default=0)\n");
    printf("-l, \tLpr confidence((float: 0-1), default=0.45)\n");
    ...
    if (pstConfig->nSize > 0 && pstConfig->pstItems) {
        for (size_t i = 0; i < pstConfig->nSize; i++) {
            if (pstConfig->pstItems[i].pstrType && pstConfig->pstItems[i].pstrValue) {
                ...
                else if (strcmp(pstConfig->pstItems[i].pstrType, "lpr_confidence") == 0) {
                    if (pstConfig->pstItems[i].nValueSize == sizeof(AX_SKEL_COMMON_THRESHOLD_CONFIG_T)) {
                        AX_SKEL_COMMON_THRESHOLD_CONFIG_T *pstConf =
                            (AX_SKEL_COMMON_THRESHOLD_CONFIG_T *)pstConfig->pstItems[i].pstrValue;
                        ALOGI("SKEL get %s: %f", pstConfig->pstItems[i].pstrType, pstConf->fValue);
...
AX_S32 main(AX_S32 argc, AX_CHAR *argv[]) {
    ...
    const AX_CHAR *frameRateStr = NULL;
    ...
    AX_F32 fLprConfidence = 0.45;
    AX_F32 fPushQualityThreshold = 0.0;
    AX_F32 fFacePosePitch = 180.0;
    AX_F32 fFacePoseYaw = 180.0;
    AX_F32 fFacePoseRoll = 180.0;
    AX_F32 fFaceBlur = 1.0;
    AX_U32 nFaceFeatureAnalyzer = 0;
    ...
    while ((c = getopt(argc, argv, "i:r:I:F:w:o:m:t:d:u:p:j:v:f:c:l:N:H:V:C:Q:P:Y:R:B:h::")) != -1) {
        isExit = 0;
        switch (c) {
            case 'i':
            ...
            case 'I':
                ...
                break;
            case 'F':
                frameRateStr = (const AX_CHAR *)optarg;
                break;
            case 'w':
            ...
            case 'j':
                ...
                break;
            case 'f':
                nFaceFeatureAnalyzer = atof(optarg);
                break;
            case 'c':
                fConfidence = atof(optarg);
                break;
            case 'l':
                fLprConfidence = atof(optarg);
                break;
            case 'Q':
                fPushQualityThreshold = atof(optarg);
                break;
            case 'P':
                fFacePosePitch = atof(optarg);
                break;
            case 'Y':
                fFacePoseYaw = atof(optarg);
                break;
            case 'R':
                fFacePoseRoll = atof(optarg);
                break;
            case 'B':
                fFaceBlur = atof(optarg);
                break;
            case 'H':
    ...
    if (SaveResultPath) {
        ...
        } else if (nPPL == AX_SKEL_PPL_PET) {
            RUN_COMMAND("mkdir -p %s/%s", SaveResultPath, SKEL_SAMPLE_OUTPUT_PET_PATH);
        } else if (nPPL == AX_SKEL_PPL_GESTURE) {
            RUN_COMMAND("mkdir -p %s/%s", SaveResultPath, SKEL_SAMPLE_OUTPUT_GESTURE_PATH);
        } else if (nPPL == AX_SKEL_PPL_PACKAGE) {
            RUN_COMMAND("mkdir -p %s/%s", SaveResultPath, SKEL_SAMPLE_OUTPUT_PACKAGE_PATH);
        } else if (nPPL == AX_SKEL_PPL_EBIKE) {
            RUN_COMMAND("mkdir -p %s/%s", SaveResultPath, SKEL_SAMPLE_OUTPUT_EBIKE_PATH);
        } else {
    ...
    if (frameRateStr) {
        AX_CHAR *temp_p = strstr(frameRateStr, ":");
        ...
        nSrcFrameRate = atoi(frameRateStr);
        nDstFrameRate = atoi(temp_p + 1);
    }
...
#if 0
    // rebuild init param
    AX_SKEL_INIT_PARAM_T stInitParamRebuild = {0};
    stInitParamRebuild.pStrModelDeploymentPath = stInitParam.pStrModelDeploymentPath;
    stConfig.pstItems[itemIndex].pstrType = (AX_CHAR *)"init_param_rebuild";
    stConfig.pstItems[itemIndex].pstrValue = (AX_VOID *)&stInitParamRebuild;
    stConfig.pstItems[itemIndex].nValueSize = sizeof(AX_SKEL_INIT_PARAM_T);
    itemIndex++;
#endif
    ...
    // detect + track (disable push)
    ...
    else if (nDetectType == 2) {
        // track_enable
        stConfig.pstItems[itemIndex].pstrType = (AX_CHAR *)"track_enable";
        stTrackEnableThreshold.fValue = 1;
        stConfig.pstItems[itemIndex].pstrValue = (AX_VOID *)&stTrackEnableThreshold;
        stConfig.pstItems[itemIndex].nValueSize = sizeof(AX_SKEL_COMMON_THRESHOLD_CONFIG_T);
        itemIndex++;
        ...
        // push_enable
        stConfig.pstItems[itemIndex].pstrType = (AX_CHAR *)"push_enable";
        stPushEnableThreshold.fValue = 1;
        stConfig.pstItems[itemIndex].pstrValue = (AX_VOID *)&stPushEnableThreshold;
        stConfig.pstItems[itemIndex].nValueSize = sizeof(AX_SKEL_COMMON_THRESHOLD_CONFIG_T);
        itemIndex++;
    }
    ...
    stTarget.nSize = 1;
    AX_SKEL_TARGET_ITEM_T stTargetItem = {0};
    stTargetItem.pstrObjectCategory = "body";
    stTarget.pstItems = &stTargetItem;
    ...
    stPushTarget.nSize = 1;
    AX_SKEL_TARGET_ITEM_T stPushTargetItem = {0};
    stPushTargetItem.pstrObjectCategory = "body";
    stPushTarget.pstItems = &stPushTargetItem;

    stConfig.pstItems[itemIndex].pstrValue = (AX_VOID *)&stPushTarget;
    stConfig.pstItems[itemIndex].nValueSize = sizeof(AX_SKEL_TARGET_CONFIG_T);
    itemIndex++;
#endif
    ...
    stPushAttrAlways.bEnable = AX_FALSE;
    ...
    // default: HVCP(AX_SKEL_ANALYZER_ATTR_PLATE_ATTRIBUTE); FACE(AX_SKEL_ANALYZER_ATTR_FACE_ATTRIBUTE)
    AX_SKEL_ANALYZER_CONFIG_T stAnalyze = {0};
    AX_SKEL_ANALYZER_ATTR_E analyze_attribute[] = {AX_SKEL_ANALYZER_ATTR_FACE_FEATURE, AX_SKEL_ANALYZER_ATTR_FACE_ATTRIBUTE, AX_SKEL_ANALYZER_ATTR_PLATE_ATTRIBUTE};

    if (nFaceFeatureAnalyzer == 1) {
        stConfig.pstItems[itemIndex].pstrType = (AX_CHAR *)"analyzer_attr_config";
        stAnalyze.peItems = &analyze_attribute[0];
        stAnalyze.nSize = sizeof(analyze_attribute)/sizeof(analyze_attribute[0]);
    ...
    // set config
    {
        ...
        // lpr_confidence
        stConfig.pstItems[itemIndex].pstrType = (AX_CHAR *)"lpr_confidence";
        AX_SKEL_COMMON_THRESHOLD_CONFIG_T stLprConfidence = {0};
        stLprConfidence.fValue = fLprConfidence;
        stConfig.pstItems[itemIndex].pstrValue = (AX_VOID *)&stLprConfidence;
        stConfig.pstItems[itemIndex].nValueSize = sizeof(AX_SKEL_COMMON_THRESHOLD_CONFIG_T);
        itemIndex++;
        ...
        stBodyAttrFilter.stCommonAttrFilterConfig.fQuality = fPushQualityThreshold;
        ...
        stVehicleAttrFilter.stCommonAttrFilterConfig.fQuality = fPushQualityThreshold;
        ...
        stCycleAttrFilter.stCommonAttrFilterConfig.fQuality = fPushQualityThreshold;
        ...
        stPlateAttrFilter.stCommonAttrFilterConfig.fQuality = fPushQualityThreshold;
        ...
        stFaceAttrFilter.stFaceAttrFilterConfig.stPoseblur.fPitch = fFacePosePitch;
        stFaceAttrFilter.stFaceAttrFilterConfig.stPoseblur.fYaw = fFacePoseYaw;
        stFaceAttrFilter.stFaceAttrFilterConfig.stPoseblur.fRoll = fFacePoseRoll;
        stFaceAttrFilter.stFaceAttrFilterConfig.stPoseblur.fBlur = fFaceBlur;
        stConfig.pstItems[itemIndex].nValueSize = sizeof(AX_SKEL_ATTR_FILTER_CONFIG_T);
    ...
    for (AX_U32 nRepeat = 0; nRepeat < nRepeatTimes; nRepeat++) {
        ...
        for (AX_U32 nFrameCnt = 0; nFrameCnt < InputFileFrameCnt; nFrameCnt++) {
            AX_POOL poolId = AX_INVALID_POOLID;
            ...
            if (InputFileFrameCnt != 1) {
                if (FrameSkipCtrl(nSrcFrameRate, nDstFrameRate, nSkelFrameId)) {
                    ...
                    if (!FrameMgrGet(&YUVDataPhy, &YUVDataVir, nFrameSize, nSkelFrameId, &poolId, &blkId)) {
                    ...
                    if (nOneSize != nFrameSize) {
                        if (!OneYUVDataVir) {
                            AX_SYS_MemAlloc(&OneYUVDataPhy, (AX_VOID **)&OneYUVDataVir, nOneSize, 256, (AX_S8 *)"SKEL_TEST");
                        }
                ...
                FrameMgrGet(&YUVDataPhy, &YUVDataVir, nFrameSize, nSkelFrameId, &poolId, &blkId);
            ...
            while (1) {
                ...
                if (nRet == 0) {
                    break;
                } else if (nRet == AX_ERR_SKEL_QUEUE_FULL) {
                    usleep(1000);
                    continue;
                } else if (nRet != 0) {
                    ALOGE("AX_SKEL_SendFrame, ret = 0x%x", nRet);
                    goto EXIT3;
                }
            }
            ...
            for (AX_U32 i = 0; i < pstResult->nObjectSize; i++) {
                ...
                ALOGN("\t\tTrackId: %lld, TrackState: %d, FrameId: %lld", pstItems->nTrackId, pstItems->eTrackState, pstItems->nFrameId);

                OUTPUT_LOG_SAVE("\tTrackId: %lld, TrackState: %d, FrameId: %lld", pstItems->nTrackId, pstItems->eTrackState, pstItems->nFrameId);

                if (nPPL == AX_SKEL_PPL_FACE) {
                    ALOGN("\t\tRect[%d] %s: [%f, %f, %f, %f], Confidence: %f, Pitch: %f, Yaw: %f, Roll: %f, Blur: %f", i, pstItems->pstrObjectCategory,
                        pstItems->stRect.fX,
                        pstItems->stRect.fY, pstItems->stRect.fW,
                        pstItems->stRect.fH, pstItems->fConfidence,
                        pstItems->stFaceAttr.stPoseBlur.fPitch, pstItems->stFaceAttr.stPoseBlur.fYaw,
                        pstItems->stFaceAttr.stPoseBlur.fRoll, pstItems->stFaceAttr.stPoseBlur.fBlur);
                } else {
                ...
                if ((pstItems->eTrackState == AX_SKEL_TRACK_STATUS_NEW
                    ...
                    if (nPPL == AX_SKEL_PPL_FACE) {
                        OUTPUT_LOG_SAVE("\t\tObject[%d] %s: [%f, %f, %f, %f], Confidence: %f, Pitch: %f, Yaw: %f, Roll: %f, Blur: %f",
                    ...
                    //face attr
                    if(nPPL == AX_SKEL_PPL_FACE && pstItems->bFaceAttr){
                        ALOGI("\t\tFace Attribute:");
                        ALOGI("\t\t\tyaw %f,",pstItems->stFaceAttr.stPoseBlur.fYaw);
                        ALOGI("\t\t\tpitch %f,",pstItems->stFaceAttr.stPoseBlur.fPitch);
                        ALOGI("\t\t\troll %f,",pstItems->stFaceAttr.stPoseBlur.fRoll);
                        ALOGI("\t\t\tblur %f,",pstItems->stFaceAttr.stPoseBlur.fBlur);
                        ALOGI("\t\t\tmask %d,",pstItems->stFaceAttr.nMask);
                ...
                if (AX_SKEL_TRACK_STATUS_SELECT == pstItems->eTrackState) {
                    ...
                    if (pstItems->bCropFrame
                        ...
                        if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_FACE) {
                            sprintf(arrDat, "frame%lld_crop_%s[%lld]_Gender[%s]_Age[%d]_Mask[%s]_Score[%f]_Blur[%f]_%dx%d",
                                    ...
                                    pstItems->fConfidence,
                                    pstItems->stFaceAttr.stPoseBlur.fBlur,
                            ...
                            sprintf(arrDat, "frame%lld_crop_%s[%lld]_Valid[%d]_Num[%s]_Color[%s]_Score[%f]_Conf[%f]_%dx%d",
                                    pstItems->stCropFrame.nFrameId,
                                    pstItems->pstrObjectCategory,
                                    pstItems->nTrackId,
                            ...
                            sprintf(arrDat, "frame%lld_crop_%s[%lld]_plate_Valid[%d]_Num[%s]_Color[%s]_Score[%f]_Conf[%f]_%dx%d",
                                    pstItems->stCropFrame.nFrameId,
                                    pstItems->pstrObjectCategory,
                                    pstItems->nTrackId,
                                    AttrInfo.tPlateInfo.bValid,
                        ...
                        else if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_GESTURE) {
                            sprintf(arrDat, "frame%lld_crop_%s[%lld]_Type[%s]_Score[%f]_%dx%d",
                                    pstItems->stCropFrame.nFrameId,
                                    pstItems->pstrObjectCategory,
                                    pstItems->nTrackId,
                                    AttrInfo.tGdInfo.szType,
                                    AttrInfo.tGdInfo.fScore,
                                    pstItems->stCropFrame.nFrameWidth,
                                    pstItems->stCropFrame.nFrameHeight);

                            OUTPUT_LOG_SAVE("\t\t[GESTURE ATTR] %s", arrDat);
                        }
                        else if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_EBIKE) {
                            sprintf(arrDat, "frame%lld_crop_%s[%lld]_Type[%s]_Score[%f]_%dx%d",
                                    pstItems->stCropFrame.nFrameId,
                                    pstItems->pstrObjectCategory,
                                    pstItems->nTrackId,
                                    AttrInfo.tEbikeInfo.szType,
                                    AttrInfo.tEbikeInfo.fScore,
                                    pstItems->stCropFrame.nFrameWidth,
                                    pstItems->stCropFrame.nFrameHeight);

                            OUTPUT_LOG_SAVE("\t\t[EBIKE ATTR] %s", arrDat);
                        }
                        else if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_PACKAGE) {
                            sprintf(arrDat, "frame%lld_crop_%s[%lld]_Type[%s]_Score[%f]_%dx%d",
                                    pstItems->stCropFrame.nFrameId,
                                    pstItems->pstrObjectCategory,
                                    pstItems->nTrackId,
                                    AttrInfo.tPackageInfo.szType,
                                    AttrInfo.tPackageInfo.fScore,
                                    pstItems->stCropFrame.nFrameWidth,
                                    pstItems->stCropFrame.nFrameHeight);

                            OUTPUT_LOG_SAVE("\t\t[PACKAGE ATTR] %s", arrDat);
                        }
                        else {
                    ...
                    if (pstItems->bPanoraFrame
                        ...
                        AX_CHAR strFile[512] = {0};
                        if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_FACE) {
                            sprintf(arrDat, "frame%lld_panora_%s[%lld]_Gender[%s]_Age[%d]_Mask[%s]_Score[%f]_Blur[%f]_%dx%d.jpg",
                                    ...
                                    pstItems->fConfidence, pstItems->stFaceAttr.stPoseBlur.fBlur,
                                    pstItems->stCropFrame.nFrameWidth, pstItems->stCropFrame.nFrameHeight);
                        }
                        else if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_PLATE) {
                            sprintf(arrDat, "frame%lld_panora_%s[%lld]_Valid[%d]_Num[%s]_Color[%s]_Score[%f]_Conf[%f]_%dx%d.jpg",
                                    pstItems->stCropFrame.nFrameId, pstItems->pstrObjectCategory, pstItems->nTrackId,
                                    AttrInfo.tPlateInfo.bValid, AttrInfo.tPlateInfo.szNum, AttrInfo.tPlateInfo.szColor, AttrInfo.tPlateInfo.fScore,
                                    pstItems->fConfidence,
                                    pstItems->stCropFrame.nFrameWidth, pstItems->stCropFrame.nFrameHeight);
                        }
                        else if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_VEHICLE) {
                            sprintf(arrDat, "frame%lld_panora_%s[%lld]_plate_Valid[%d]_Num[%s]_Color[%s]_Score[%f]_Conf[%f]_%dx%d.jpg",
                                    pstItems->stCropFrame.nFrameId, pstItems->pstrObjectCategory, pstItems->nTrackId,
                                    AttrInfo.tPlateInfo.bValid, AttrInfo.tPlateInfo.szNum, AttrInfo.tPlateInfo.szColor, AttrInfo.tPlateInfo.fScore,
                                    pstItems->fConfidence,
                                    pstItems->stCropFrame.nFrameWidth, pstItems->stCropFrame.nFrameHeight);
                            ...
                            sprintf(arrDat, "frame%lld_panora_%s[%lld]_Type[%s]_Score[%f]_%dx%d.jpg",
                                    pstItems->stCropFrame.nFrameId, pstItems->pstrObjectCategory, pstItems->nTrackId,
                                    AttrInfo.tGdInfo.szType, AttrInfo.tGdInfo.fScore,
                                    pstItems->stCropFrame.nFrameWidth, pstItems->stCropFrame.nFrameHeight);
                        }
                        else if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_EBIKE) {
                            sprintf(arrDat, "frame%lld_panora_%s[%lld]_Type[%s]_Score[%f]_%dx%d.jpg",
                                    pstItems->stCropFrame.nFrameId, pstItems->pstrObjectCategory, pstItems->nTrackId,
                                    AttrInfo.tEbikeInfo.szType, AttrInfo.tEbikeInfo.fScore,
                                    pstItems->stCropFrame.nFrameWidth, pstItems->stCropFrame.nFrameHeight);
                        }
                        else if (AttrInfo.bExist && AttrInfo.eType == ATTR_TYPE_PACKAGE) {
                            sprintf(arrDat, "frame%lld_panora_%s[%lld]_Type[%s]_Score[%f]_%dx%d.jpg",
                                    pstItems->stCropFrame.nFrameId, pstItems->pstrObjectCategory, pstItems->nTrackId,
                                    AttrInfo.tPackageInfo.szType, AttrInfo.tPackageInfo.fScore,
                                    pstItems->stCropFrame.nFrameWidth, pstItems->stCropFrame.nFrameHeight);
                        }
                        else {
                ...
                if (AX_SKEL_TRACK_STATUS_SELECT == pstItems->eTrackState
                    && pstItems->pstFeatureItem
                    && 0 < pstItems->nFeatureSize
                    && SaveResultPath) {
                    AX_CHAR strFile[256] = {0};
                    sprintf(strFile, "%s/%s/frame[%lld]_feature_%s[%lld].db",
                        SaveResultPath, pstItems->pstrObjectCategory,
                        pstItems->stCropFrame.nFrameId,
                        pstItems->pstrObjectCategory, pstItems->nTrackId);
                    FILE *fp_w = fopen(strFile, "wb");

                    if (fp_w) {
                        ALOGI("\t\t\tWrite feature to file: %s, nDataSize: %d",
                                strFile,
                                pstItems->pstFeatureItem[0].nDataSize);
                        fwrite((AX_U8 *)pstItems->pstFeatureItem[0].pData, 1, pstItems->pstFeatureItem[0].nDataSize, fp_w);
                        fclose(fp_w);
                    }
                }
