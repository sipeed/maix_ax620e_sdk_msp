...
static AX_VOID *SAMPLE_VO_PLAY_THREAD(AX_VOID *pData)
{
    ...
    for (i = 0; i < u32FrameMax; i++)
        AX_POOL_ReleaseBlock(stFrames[i].u32BlkId[0]);
...
static AX_S32 pwm_set_value(AX_U8 pwm_num, AX_U32 period, AX_U32 duty_cycle)
{
    FILE *fp = NULL;
	AX_U8 pwmGroup;
    char file_name[50];
	...
	pwmGroup = (pwm_num/4)*4;

    sprintf(file_name, "/sys/class/pwm/pwmchip%d/pwm%d", pwmGroup, pwm_num%4);
    if (0 != access(file_name, F_OK)) {
        sprintf(file_name, "/sys/class/pwm/pwmchip%d/export", pwmGroup);
        fp = fopen(file_name, "w");
        if (fp == NULL) {
            printf("Cannot open %s.\n", file_name);
            return -1;
        }
        fprintf(fp, "%d", pwm_num%4);
        fclose(fp);

        sprintf(file_name, "/sys/class/pwm/pwmchip%d/pwm%d/period", pwmGroup, pwm_num%4);
        fp = fopen(file_name, "w");
        if (fp == NULL) {
            printf("Cannot open %s.\n", file_name);
            return -1;
        ...
        sprintf(file_name, "/sys/class/pwm/pwmchip%d/pwm%d/duty_cycle", pwmGroup, pwm_num%4);
        fp = fopen(file_name, "w");
        if (fp == NULL) {
            printf("Cannot open %s.\n", file_name);
            return -1;
...
static AX_S32 pwm_enable(AX_U8 pwm_num, AX_U8 enable)
{
    FILE *fp = NULL;
	AX_U8 pwmGroup;
    char file_name[50];
	...
	sprintf(file_name, "/sys/class/pwm/pwmchip%d/pwm%d/enable", pwmGroup, pwm_num%4);
	fp = fopen(file_name, "w");
	if (fp == NULL) {
        printf("Cannot open %s.\n", file_name);
        return -1;
...
exit0:
    for (i = 0; i < pstVoConf->u32LayerNr; i++) {
        pstVoLayerConf = &pstVoConf->stVoLayer[i];
        s32Ret = SAMPLE_VO_POOL_DESTROY(pstVoLayerConf->u32ChnPoolId);\
        if (s32Ret)
            SAMPLE_PRT("chnPoolId:%d pool destory fail! s32Ret:0x%x!\n", pstVoLayerConf->u32ChnPoolId, s32Ret);

        s32Ret = SAMPLE_VO_POOL_DESTROY(pstVoLayerConf->u32LayerPoolId);
        if (s32Ret)
            SAMPLE_PRT("layerPoolId:%d pool destory fail s32Ret:0x%x!\n", pstVoLayerConf->u32LayerPoolId, s32Ret);
    }
