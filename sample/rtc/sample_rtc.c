...
OPEN_DEV:
    ...
    do{
        ...
        usleep(300000); /*wait 300ms for rtc update*/
        gettimeofday(&tv_start, NULL);
        fprintf(stderr, "After set current RTC date/time is %d-%d-%d, %02d:%02d:%02d.\n",
                rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
                rtc_tm.tm_hour, rtc_tm.tm_min, rtc_tm.tm_sec);

        /* Set the alarm to 15 secs in the future, and check for rollover */
        rtc_time = rtc_tm2time(&rtc_tm);
        saved_time1 = rtc_time;
        ...
        fprintf(stderr, "Alarm time will set to: %d-%d-%d, %02d:%02d:%02d.\n",
                rtc_tm.tm_year + 1900, rtc_tm.tm_mon + 1, rtc_tm.tm_mday,
                rtc_tm.tm_hour, rtc_tm.tm_min, rtc_tm.tm_sec);
