...
/*
    FillPureData()
    @nColor: YUV:#YYUUVV; RGB:#RRGGBB
*/
static AX_VOID FillPureData(AX_U32 u32Width, AX_U32 u32Height, AX_U32 u32Stride,
                            AX_U8 *u8YMem, AX_U8 *u8UVMem, AX_U32 nFormat, AX_U32 nColor)
{
    AX_U32 ii;

    switch (nFormat) {
        case  AX_FORMAT_YUV420_SEMIPLANAR:
        for (ii = 0; ii < u32Height * u32Stride; ii++)
            u8YMem[ii] = (nColor & 0xFF) >> 16;

        for (ii = 0; ii < u32Height * u32Stride / 2; ii+=2)
        {
            u8UVMem[ii] = (nColor & 0xFF) >> 8;
            u8UVMem[ii + 1] = (nColor & 0xFF);
        }
        break;
    case  AX_FORMAT_YUV420_SEMIPLANAR_VU:
        ...
        for (ii = 0; ii < u32Height * u32Stride / 2; ii+=2)
        {
            u8UVMem[ii] = nColor & 0xFF;
            u8UVMem[ii + 1] = (nColor & 0xFF) >> 8;
        }
        break;
    case AX_FORMAT_RGB888:
        for (ii = 0; ii < u32Height * u32Stride * 3; ii++) {
            u8YMem[ii] = (nColor & 0xFF) >> 16;
            u8YMem[ii + 1] = (nColor & 0xFF) >> 8;
            u8YMem[ii + 2] = (nColor & 0xFF);
        }
    default:
        printf("Not support format:0x%x", nFormat);
        break;
    }
}
...
static void FillColorBarRGB24(AX_U8 *rgb, AX_U32 w, AX_U32 h, AX_U32 stride)
{
	AX_U32 barW;
	AX_U32 i = 0, j = 0;
...
AX_S32 PatternSmpteFill(AX_IMG_FORMAT_E enPixFmt, AX_U32 u32Width, AX_U32 u32Height, AX_U32 u32Stride, AX_U8 *u8Mem)
{
    ...
    case AX_FORMAT_YUV400:
        FillSmpteYUV(u32Width, u32Height, u32Stride, u8Mem, NULL, NULL);
        break;
    case AX_FORMAT_YUV420_SEMIPLANAR:
    case AX_FORMAT_YUV420_SEMIPLANAR_VU:
