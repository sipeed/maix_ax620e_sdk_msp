...
AX_VOID IVPS_ObjHelp(AX_VOID)
{
    ...
    printf("\t--mosaic       (optional) : draw mosaic API test, 0:TDP\n");
    ...
    printf("\t--pool_type    (optional) : select pool type, value{0(default): common pool; 1: private pool; 2: user pool}\n");
    printf("\t--few_pool     (optional) : set few common pool to test retry get vb\n");
    printf("\t--ch           (optional) : output channel width and height, pattern{WxH}\n");
    printf("\t--crop         (optional) : crop info, pattern{X0:Y0:W:H}\n");
    printf("\t--resize       (optional) : resize attribute, pattern{scl type:aspect mode:back color(0xYYUUVV)[:output format]}\n");
    printf("\t--angle        (optional) : rotation angle, value{0:0; 1:90; 2:180; 3:270}\n");
    printf("\t--flip         (optional) : flip or mirror, value{0:no flip; 1:mirror; 2:flip; 3:flip and mirror}\n");
    printf("\t--format       (optional) : color format, see as below\n");
    printf("\t--alpha        (optional) : image alpha, scope:[0, 255]\n");
    printf("\t--X0           (optional) : output position horizontal axis\n");
    printf("\t--Y0           (optional) : output position vertical axis\n");
    printf("\t--meshfile     (optional) : file path of mesh table\n");
    printf("\t--meshwidth    (optional) : block width of mesh table\n");
    printf("\t--meshheight   (optional) : block height of mesh table\n");
    printf("\t--MD5          (optional) : MD5 check with UT case ID\n");
    printf("\t--h            (optional) : help\n");

    printf("\t    -v  <PicPath>@<Format>@<Stride>x<Height>@<CropW>x<CropH>[+<CropX0>+<CropY0>][#<FbcLevel>]>\n");
    printf("\t   e.g: -v /opt/bin/data/ivps/800x480car.nv12@3@800x480@600x400+100+50 \n\n");
    printf("\t   [-g] <PicPath>@<Format>@<Stride>x<Height>[+<DstX0>+<DstY0>]>\n");
    printf("\t   e.g: -g /opt/bin/data/ivps/rgb400x240.rgb24@161@400x240+100+50 \n\n");
    printf("\t   [-n] <repeat num>]\n");
    printf("\t   [-r] <region num>]\n\n");
    ...
    printf("\nExample1: Pipeline, Src:1920x1080(NV12)\n");
    printf("\t%s -v /opt/data/ivps/1920x1080.nv12@3@1920x1080@1920x1080 -n 1 --save /opt/data\n", SAMPLE_NAME);
    printf("\nExample2: SyncAPI, TDP, Src:800x1280(ARGB8888), Dst:1280x800(ARGB8888), rotate 90 degree\n");
    printf("\t%s -v /1@199@800x1280@800x1280 --fliprotation 0 --angle 1 --save /opt/data\n", SAMPLE_NAME);
    printf("\nExample3: SyncAPI, TDP, Src:1920x1080(NV12), Dst:1920x1080(YUYV)\n");
    printf("\t%s -v /1@3@1920x1080@1920x1080 --csc 0 --format 13 --save /opt/data\n", SAMPLE_NAME);
    printf("\nExample4: Pipeline, GDC, Mesh, Src:2560x1440(NV12), Dst:2560x1440(NV12), \n");
    printf("\t%s -v /opt/data/2560x1440-tv.nv12@3@2560x1440@2560x1440 -n 1 --meshfile /opt/data/mesh/mesh_tv_2560x1440_80x48.txt --meshwidth 80 --meshheight 48 --save /opt/data\n", SAMPLE_NAME);
}
