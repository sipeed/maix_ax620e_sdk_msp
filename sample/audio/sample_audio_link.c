...
static AX_S16 gMsInSndCardBuf = 0;
...
static void *AiAedRecvThread(void *arg)
{
    ...
    while (!gLoopExit) {
        ...
        if (ret) {
            printf("AX_AI_GetAedResult audio_failed: %x\n", ret);
            break;
        }
...
static void *AoCtrlThread(void *arg)
{
    ...
    if (ret) {
        printf("AX_AO_DisableDev audio_failed! ret= %x\n",ret);
    }
...
static void *AencRecvThread(void *arg)
{
    ...
    while (1) {
        ...
        if (ret) {
            printf("AX_AENC_GetStream audio_failed: %x\n", ret);
            break;
        }
        getNumber++;
...
static void *AsyncTestThread(void *arg)
{
    ...
    if (!strcmp(gAsyncTestName, "PauseResume")) {
        ...
        for (int i = 0; i < gAsyncTestNumber; i++) {
            ...
            if (ret) {
                printf("AX_AO_PauseRecvFrame audio_failed! ret = %x", ret);
            }
            ret = AX_AO_ResumeRecvFrame(gCardNum, gDeviceNum);
            if (ret) {
                printf("AX_AO_ResumeRecvFrame audio_failed! ret = %x", ret);
            }
        }
    } else {
...
static void PrintHelp()
{
    ...
    printf("up_vqe:                  test up_vqe.\n");
    printf("dn_vqe:                  test dn_vqe.\n");
    printf("args:\n");
    ...
    printf("  --msInSndCardBuf:       aec delay ms.               (support short), default: 0\n");
...
static int AudioInput()
{
    ...
    ret = AX_SYS_Init();
    if (AX_SUCCESS != ret) {
        printf("AX_SYS_Init audio_failed! Error Code:0x%X\n", ret);
        return -1;
    }
    ...
    if (PoolId == AX_INVALID_POOLID) {
        printf("AX_POOL_CreatePool audio_failed!\n");
        goto FREE_SYS;
    }
    ...
    ret = AX_AI_Init();
    if (ret) {
        printf("AX_AI_Init audio_failed! Error Code:0x%X\n", ret);
        goto DESTROY_POOL;
    }
    ...
    if(ret){
        printf("AX_AI_SetPubAttr audio_failed! ret = %x\n", ret);
        goto AI_DEINIT;
    }

    ret = AX_AI_AttachPool(card,device,PoolId);
    if(ret){
        printf("AX_AI_AttachPool audio_failed! ret = %x\n", ret);
        goto AI_DEINIT;
    }
    ...
    stVqeAttr.msInSndCardBuf = gMsInSndCardBuf;
    ...
    if (IsUpTalkVqeEnabled(&stVqeAttr)) {
        ...
        if(ret){
            printf("AX_AI_SetUpTalkVqeAttr audio_failed! ret = %x\n",ret);
            goto AI_DETACHPOOL;
        }
    ...
    if (gHpfCfg.bEnable) {
        ...
        if(ret){
            printf("AX_ACODEC_RxHpfSetAttr audio_failed! ret = %x\n", ret);
            goto AI_DETACHPOOL;
        }
        ret = AX_ACODEC_RxHpfEnable(card);
        if(ret){
            printf("AX_ACODEC_RxHpfEnable audio_failed! ret = %x\n", ret);
            goto AI_DETACHPOOL;
        }
    }
    if (gLpfCfg.bEnable) {
        ...
        if(ret){
            printf("AX_ACODEC_RxLpfSetAttr audio_failed! ret = %x\n", ret);
            goto DIS_HPF;
        }
        ret = AX_ACODEC_RxLpfEnable(card);
        if(ret){
            printf("AX_ACODEC_RxLpfEnable audio_failed! ret = %x\n", ret);
            goto DIS_HPF;
        }
    }
    if (gEqCfg.bEnable) {
        ...
        if(ret){
            printf("AX_ACODEC_RxEqSetAttr audio_failed! ret = %x\n", ret);
            goto DIS_LPF;
        }
        ret = AX_ACODEC_RxEqEnable(card);
        if(ret){
            printf("AX_ACODEC_RxEqEnable audio_failed! ret = %x\n", ret);
            goto DIS_LPF;
        }
    }
    ...
    if (ret){
        printf("AX_AI_EnableDev audio_failed! ret = %x \n",ret);
        goto DIS_EQ;
    }
    ...
    if (gResample) {
        ...
        if(ret){
            printf("AX_AI_EnableResample audio_failed! ret = %x,\n",ret);
            goto DIS_AI_DEVICE;
        }
        outRate = gResRate;
    }
    ...
    if(ret){
        printf("AX_AI_SetVqeVolume audio_failed! ret = %x\n", ret);
        goto DIS_AI_DEVICE;
    }
    ...
    if (gSaveFile) {
        ...
        if(ret){
            printf("AX_AI_SaveFile audio_failed! ret = %x\n", ret);
            goto DIS_AI_DEVICE;
        }
    }
    ...
    if (gDbDetection) {
        ...
        if(ret){
            printf("AX_AI_SetAedAttr audio_failed! ret = %x\n", ret);
            goto DIS_AI_DEVICE;
        }

        ret = AX_AI_EnableAed(card, device);
        if(ret){
            printf("AX_AI_EnableAed audio_failed! ret = %x\n", ret);
            goto DIS_AI_DEVICE;
        }
    ...
    while (1) {
        ...
        if (ret != AX_SUCCESS) {
            printf("AX_AI_GetFrame audio_failed, ret: %x\n",ret);
            break;
        }
        getNumber++;
        if (gWriteFrames)
        ...
        if (ret) {
            printf("AX_AI_ReleaseFrame audio_failed! ret=%x\n",ret);
        }
    ...
    if (gDbDetection) {
       ret = AX_AI_DisableAed(card, device);
        if(ret){
            printf("AX_AI_DisableAed audio_failed! ret= %x\n",ret);
            goto FREE_SYS;
        }
    }
...
FREE_SYS:
    ret = AX_SYS_Deinit();
    if (AX_SUCCESS != ret) {
        printf("AX_SYS_Deinit audio_failed! Error Code:0x%X\n", ret);
        ret = -1;
    }
...
static int Audio_UpVqeTest()
{
    ...
    AX_VOID *in_data = NULL;
    AX_VOID *out_data = NULL;
    AX_VOID *ref_data = NULL;
    AX_VOID *data = NULL;
    unsigned int totalFrames = 0;
    FILE * output_file = NULL;
    ...
    ref_data = malloc(size);
    data = malloc(size);
    if (!in_data || !out_data || !ref_data || !data) {
    ...
    memcpy(&stVqeAttr.stAgcCfg, &gAgcCfg, sizeof(AX_AGC_CONFIG_T));
    memcpy(&stVqeAttr.stAecCfg, &gAecCfg, sizeof(AX_AEC_CONFIG_T));
    AX_VOID *pTmpVqeCtx = NULL;

    if (!IsUpTalkVqeEnabled(&stVqeAttr)) {
        goto FREE;
    }

    pTmpVqeCtx = AX_AP_UpTalkVqe_Create(&stVqeAttr);
    if (!pTmpVqeCtx) {
        printf("UpTalkVqe creat audio_failed.)\n");
        goto FREE;
    }

    read_size = stVqeAttr.u32FrameSamples * num_channels * bits_per_sample / 8;
    if (read_size > size) {
        printf("size(%d) shoule be larger than read_size(%d).\n", size, read_size);
        goto DESTROY;
    }

    //vqe process
    do {
            num_read = fread(data, 1, read_size , input_file);
            AX_AUDIO_InterleavedToNoninterleaved16(data, stVqeAttr.u32FrameSamples, in_data, ref_data);
            if (num_read == read_size) {
                AX_AP_UpTalkVqe_Process(pTmpVqeCtx, in_data, ref_data, out_data);
                if (gWriteFrames) {
                    fwrite(out_data, 2, stVqeAttr.u32FrameSamples, output_file);
                }
                totalFrames += stVqeAttr.u32FrameSamples;
            } else {
                break;
            }
    }while (!gLoopExit);

    if (gIsWave && gWriteFrames) {
        gChannels =  1;
        WriteWaveHeader(output_file, gChannels, sample_rate, bits_per_sample, totalFrames);
    }

    printf("up_vqe success.\n");

 DESTROY:
    ret = AX_AP_UpTalkVqe_Destroy(pTmpVqeCtx);
    if (ret) {
        printf("AX_AP_UpTalkVqe_Destroy creat audio_failed.)\n");
    }
...
static int Audio_DnVqeTest()
{
    ...
    if (num_channels != 1) {
        printf("This sample just for mono.\n");
        goto CLOSE;
    }
    ...
    pTmpVqeCtx = AX_AP_DnVqe_Create(&stVqeAttr);
    if (!pTmpVqeCtx) {
        printf("UpTalkVqe creat audio_failed.)\n");
        goto FREE;
    }
    ...
    //vqe process
    do {
            num_read = fread(in_data, 1, read_size , input_file);
            if (num_read == read_size) {
                AX_AP_DnVqe_Process(pTmpVqeCtx, in_data, out_data);
                if (gWriteFrames) {
                    fwrite(out_data, 2, stVqeAttr.u32FrameSamples, output_file);
                }
                totalFrames += stVqeAttr.u32FrameSamples;
    ...
    if (gIsWave && gWriteFrames) {
        num_channels =  1;
        WriteWaveHeader(output_file, num_channels, sample_rate, bits_per_sample, totalFrames);
    }

    printf("dn_vqe success.\n");

 DESTROY:
    ret = AX_AP_DnVqe_Destroy(pTmpVqeCtx);
    if (ret) {
        printf("AX_AP_DnVqe_Destroy creat audio_failed.)\n");
    }
...
static int AudioOutput()
{
    ...
    FILE *input_file = fopen(input_file_name, "rb");
    if (input_file == NULL) {
        printf("audio_failed to open '%s' for reading\n", input_file_name);
        return -1;
    }
    ...
    ret = AX_AO_Init();
    if (ret) {
        printf("AX_AO_Init audio_failed! Error Code:0x%X\n", ret);
        goto DESTROY_POOL;
    }
    ...
    if (ret) {
        printf("AX_AO_SetPubAttr audio_failed! ret = %x", ret);
        goto AO_DEINIT;
    }
    ...
    if (IsDnVqeEnabled(&stVqeAttr)) {
        ...
        if (ret) {
                printf("AX_AO_SetDnVqeAttr audio_failed! ret = %x \n",ret);
                goto AO_DEINIT;
        }
    }
    ...
    if (gHpfCfg.bEnable) {
        ...
        if(ret){
            printf("AX_ACODEC_TxHpfSetAttr audio_failed! ret = %x\n", ret);
            goto AO_DEINIT;
        }
        ret = AX_ACODEC_TxHpfEnable(card);
        if(ret){
            printf("AX_ACODEC_TxHpfEnable audio_failed! ret = %x\n", ret);
            goto AO_DEINIT;
        }
    }
    if (gLpfCfg.bEnable) {
        ...
        if(ret){
            printf("AX_ACODEC_TxLpfSetAttr audio_failed! ret = %x\n", ret);
            goto DIS_HPF;
        }
        ret = AX_ACODEC_TxLpfEnable(card);
        if(ret){
            printf("AX_ACODEC_TxLpfEnable audio_failed! ret = %x\n", ret);
            goto DIS_HPF;
        }
    }
    if (gEqCfg.bEnable) {
        ...
        if(ret){
            printf("AX_ACODEC_TxEqSetAttr audio_failed! ret = %x\n", ret);
            goto DIS_LPF;
        }
        ret = AX_ACODEC_TxEqEnable(card);
        if(ret){
            printf("AX_ACODEC_TxEqEnable audio_failed! ret = %x\n", ret);
            goto DIS_LPF;
        }
    }
    ...
    if (ret) {
        printf("AX_AO_EnableDev audio_failed! ret = %x \n",ret);
        goto DIS_EQ;
    }
    ...
    if (gResample) {
        ...
        if (ret) {
            printf("AX_AO_EnableResample audio_failed! ret = %x,\n",ret);
            goto DIS_AO_DEVICE;
        }
    }
    ...
    if (ret) {
        printf("AX_AO_SetVqeVolume audio_failed! ret = %x \n",ret);
        goto DIS_AO_DEVICE;
    }
    ...
    if (gSaveFile) {
        ...
        if(ret){
            printf("AX_AO_SaveFile audio_failed! ret = %x\n", ret);
            goto DIS_AO_DEVICE;
        }
    }
            ...
            if (gSimDrop) {
                if (sendCount % 30 < 15) {
                    ...
                    if (ret != AX_SUCCESS) {
                        ...
                        printf("AX_AO_SendFrame audio_failed, ret: %x\n",ret);
                        break;
                    }
                }
                if (sendCount % 250 == 0) {
    ...
    if (gInstant) {
        ...
        if (ret) {
            printf("AX_AO_ClearDevBuf audio_failed! ret = %x\n", ret);
        }
    } else {
...
static int AudioEncodeLink()
{
    ...
    memset(&stPoolConfig, 0, sizeof(AX_POOL_CONFIG_T));
    stPoolConfig.MetaSize = 8192;
    stPoolConfig.BlkSize = 15360;  /*FAAC, double mic need 8192 (1024*4*2rx) */
    stPoolConfig.BlkCnt = 33;
    stPoolConfig.IsMergeMode = AX_FALSE;
    stPoolConfig.CacheMode = AX_POOL_CACHE_MODE_NONCACHE;
    strcpy((char *)stPoolConfig.PartitionName, "anonymous");
    ...
    if (ret) {
        printf("AX_SYS_Link audio_failed! Error Code:0x%X\n", ret);
        goto DESTROY_POOL;
    }
    ...
    ret = AX_AI_AttachPool(card,device,PoolId);
    if(ret){
        printf("AX_AI_AttachPool audio_failed! ret = %x", ret);
        goto AI_DEINIT;
    }
    ...
    ret = AX_AI_EnableDev(card,device);
    if(ret){
        printf("AX_AI_EnableDev audio_failed! ret = %x", ret);
        goto AI_DETACHPOOL;
    }
    ...
    ret = AX_AENC_Init();
    if (ret) {
        printf("AX_AENC_Init audio_failed: %x\n", ret);
        goto DIS_AI_DEVICE;
    }
    const char* fileExt;
    AX_PAYLOAD_TYPE_E payloadType ;
    ...
    if (payloadType == PT_AAC) {
...
#endif
        if (ret) {
            printf("AX_AENC_AAC_Init audio_failed: %x\n", ret);
            goto DEINIT_AENC;
        }
        ...
        if (ret) {
            printf("AX_AENC_OpusInit audio_failed: %x\n", ret);
            goto DEINIT_AENC;
        }
#endif
    } else {
        pstAttr.u32PtNumPerFrm = 1024;
        pstAttr.pValue = NULL;
        if(pstAttr.u32PtNumPerFrm < stAttr.u32PeriodSize) {
            printf("ai_aenc: u32PtNumPerFrm size is too small\n");
            goto DEINIT_AENC;
        }
    }
    ...
    if (ret) {
        printf("AX_AENC_CreateChn audio_failed: %x\n", ret);
    ...
    if(ret){
        printf("AX_AENC_DestroyChn audio_failed! ret= %x\n",ret);
    }
...
#endif
        if (ret) {
            printf("AX_AENC_AAC_DeInit audio_failed! ret = %x \n",ret);
        }
        break;
    case PT_OPUS:
#ifndef CHIP_TYPE_AX620Q
        ...
        if (ret) {
            printf("AX_AENC_OpusDeInit audio_failed: %x\n", ret);
        }
#endif
        break;
    default:
...
DEINIT_AENC:
    ret = AX_AENC_DeInit();
    if(ret){
        printf("AX_AENC_DeInit audio_failed! ret= %x\n",ret);
    }
...
static int AudioDecodeLink()
{
    ...
    if (ret) {
        printf("AX_SYS_Link audio_failed!!Error ret:%x \n",ret);
        goto DESTROY_POOL;
    }
    ...
    ret = AX_ADEC_Init();
    if (ret) {
        printf("AX_ADEC_Init audio_failed: %x\n", ret);
        goto DIS_AO_DEVICE;
    }
    const char* fileExt;
    AX_PAYLOAD_TYPE_E payloadType;
...
#endif
        if (ret) {
            printf("AX_ADEC_AAC_Init audio_failed: %x\n", ret);
            goto DEINIT_ADEC;
        }
        break;
    case PT_G726:
   ...
   case PT_G723:
    ...
    case PT_OPUS:
        ...
        if (ret) {
            printf("AX_ADEC_ScOpus_Init audio_failed: %x\n", ret);
            goto DEINIT_ADEC;
        }
#endif
        break;
    ...
    if (ret) {
        printf("AX_ADEC_CreateChn audio_failed: %x\n", ret);
        goto DEINIT_ADEC_PAYLOADTYPE;
    }

    ret = AX_ADEC_AttachPool(adChn, PoolId);
    if (ret) {
        printf("AX_ADEC_AttachPool audio_failed: %x\n", ret);
        goto DESTROY_ADEC;
    }
    ...
    if (input_file == NULL) {
        printf("audio_failed to open '%s' for reading", file_path);
    ...
    if ((pstAttr.enType == PT_G711A) || (pstAttr.enType == PT_G711U) ||
        ...
        do {
            ...
            if (num_read > 0) {
                ...
                if (ret != AX_SUCCESS) {
                    printf("AX_ADEC_SendStream audio_failed: %x\n", ret);
                    break;
                }
            } else {
                loopNumber++;
...
DETACHPOOL_ADEC:
    ...
    if (ret) {
        printf("AX_ADEC_DetachPool audio_failed!ret = %x \n",ret);
    }
DESTROY_ADEC:
    ret = AX_ADEC_DestroyChn(adChn);
    if (ret) {
        printf("AX_ADEC_DestroyChn audio_failed!ret = %x \n",ret);
    }
...
#endif
        if (ret) {
            printf("AX_ADEC_AAC_DeInit audio_failed! ret = %x \n",ret);
        }
        break;
    case PT_OPUS:
#ifndef CHIP_TYPE_AX620Q
        ...
        if (ret) {
            printf("AX_ADEC_OpusDeInit audio_failed: %x\n", ret);
        }
#endif
        break;
    default:
...
DEINIT_ADEC:
    ret = AX_ADEC_DeInit();
    if (ret) {
        printf("AX_ADEC_DeInit audio_failed! ret = %x \n",ret);
    }
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        static struct option long_options[] = {
            ...
            {"msInSndCardBuf",      required_argument,  0, LONG_OPTION_MS},
            {0,                     0,                  0, 0 }
        };
    ...
    if (!strcmp(argv[optind], "ai_aenc")) {
        ...
        AudioOutput();
    } else if (!strcmp(argv[optind], "up_vqe")) {
        Audio_UpVqeTest();
    } else if (!strcmp(argv[optind], "dn_vqe")) {
        Audio_DnVqeTest();
    }else {
