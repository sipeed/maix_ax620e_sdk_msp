...
AX_S32 SAMPLE_IVES_MD_ENTRY(const AX_CHAR *pImgPath) {
    ...
    for (j = 0; j < MAX_DET_IMAGE_COUNT; ++j) {
        ...
        if (1 == j) {
            ret = AX_IVES_MD_Process(stChnAttr.mdChn, pstDetImgs[j], &stThrs, &stSad, &stBlob);
        } else {
            ret = AX_IVES_MD_Process(stChnAttr.mdChn, pstDetImgs[j], &stThrs, &stSad, NULL);
        }
        ...
        if (j > 0) {
            ...
            printf("IMAGE %lld MB SAD: %d x %d = %d\n", pstDetImgs[j]->u64SeqNum, u32MbXCount, u32MbYCount,
                   stThrs.u32Count);
            for (i = 0; i < stSad.u32Count; ++i) {
                printf("%2d ", stSad.pMbSad[i]);
                if (0 == ((i + 1) % u32MbXCount)) {
                    printf("\n");
                }
            }
