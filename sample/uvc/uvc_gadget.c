    ...
    if(NULL == dev->imgdata){
        ...
        case V4L2_PIX_FMT_H265:{
            if(img == NULL){
...
#ifdef FRAME_LOST_CHECK
                if (0 != g_uPreSeq) {
                    if (img->iSerial > g_uPreSeq + 1) {
                        printf("\nFrame lost for pre seq: %llu, current seq: %llu\n\n", g_uPreSeq, img->iSerial);
                    }
                }
                g_uPreSeq = img->iSerial;
#endif
                ...
                if (g_u64FillBuferCount < TEST_LATENCY_COUNT) {
                    g_u64FillBuferLatency += u64CurPts - img->pts;
                    g_u64FillBuferCount++;
                    if (g_u64FillBuferCount == TEST_LATENCY_COUNT) {
                        printf("===============fill buf done: avg latency: %llu\n", g_u64FillBuferLatency / TEST_LATENCY_COUNT);
...
static AX_S32 uvc_video_process(UVC_DEVICE_T *dev)
{
    ...
    if (dev->run_standalone) {
        ...
        do {
            if (dev->imgdata == NULL && dev->img_cache.img_head == dev->img_cache.img_tail) {
                return 0;
            }
            /* UVC stanalone setup. */
            ...
            if (ubuf.bytesused > PAYLOAD_HEADER_SIZE) {
                if (g_u64IoctlCount < TEST_LATENCY_COUNT) {
                    g_u64IoctlLatency += u64CurPts - uFrm64Pts;
                    g_u64IoctlCount++;
                    if (g_u64IoctlCount == TEST_LATENCY_COUNT) {
                        printf("===============ioctl done: avg latency: %llu\n", g_u64IoctlLatency / TEST_LATENCY_COUNT);
                        g_u64IoctlLatency = 0;
                        g_u64IoctlCount = 0;
                    }
                }
...
static AX_S32 UpdateSnsFps(AX_U8 nPipeId, AX_S32 nFps) {
    ...
    if (nRet != 0) {
        printf("Error, [%d]AX_ISP_GetSnsAttr failed, error: 0x%x\n", nPipeId, nRet);
        return nRet;
    }

    tSnsAttr.fFrameRate = nFps;

    nRet = AX_ISP_SetSnsAttr(nPipeId, &tSnsAttr);
    if (nRet != 0) {
        printf("Error, [%d]AX_ISP_SetSnsAttr failed, error: 0x%x\n", nPipeId, nRet);
        return nRet;
    }

    // change pipe fps ctrl
    memset(&tPipeAttr, 0x00, sizeof(tPipeAttr));
    nRet = AX_VIN_GetPipeAttr(nPipeId, &tPipeAttr);
    if (nRet != 0) {
        printf("Error, [%d]AX_VIN_GetPipeAttr failed, error: 0x%x\n", nPipeId, nRet);
        return nRet;
    }

    printf("set pipe[%d] fps to %d\n", nPipeId, nFps);

    tPipeAttr.tFrameRateCtrl.fDstFrameRate = nFps;
    tPipeAttr.tFrameRateCtrl.fSrcFrameRate = nFps;
    nRet = AX_VIN_SetPipeAttr(nPipeId, &tPipeAttr);
    if (nRet != 0) {
        printf("Error, [%d]AX_VIN_SetPipeAttr failed, error: 0x%x\n", nPipeId, nRet);
    }
...
static AX_S32 uvc_start_get_stream(UVC_DEVICE_T *dev){
    ...
    if (SAMPLE_SNS_DUMMY != dev->sns_type) {
        ...
        UpdateSnsFps(dev->pipe, dev->stream_info.u32FrameRate);

        ret = ivps_update_resolution(dev->dev_id, dev->ivps_chn, width, height, bYUY2);
        if(AX_SUCCESS != ret){
