...
#include "sample_case_triple_mipi_sns.h"
...
COMMON_SYS_POOL_CFG_T gtSysCommPoolSingleDVPIRSdr[] = {
    {256, 192, 256, AX_FORMAT_YUV422_INTERLEAVED_UYVY, 10},          /* vin nv21/nv21 use */
};
...
COMMON_SYS_POOL_CFG_T gtSysPrivatePoolSingleDVPIRSdr[] = {
    {256, 192, 256, AX_FORMAT_BAYER_RAW_10BPP, 0},         /*vin raw10 use */
};

COMMON_SYS_POOL_CFG_T gtSysPrivatePoolSingleBTSdr[] = {
    {720, 480, 720, AX_FORMAT_YUV422_INTERLEAVED_UYVY, 0}, /*vin yuv422 uyvy use */
};
...
COMMON_SYS_POOL_CFG_T gtSysCommPoolDoubleOs04a10Bt656Sdr[] = {
    {2688, 1520, 2688, AX_FORMAT_YUV420_SEMIPLANAR, 10, AX_COMPRESS_MODE_LOSSY, 4},    /* vin nv21/nv21 use */
    {720, 480, 720, AX_FORMAT_YUV422_SEMIPLANAR, 10, AX_COMPRESS_MODE_NONE, 0},    /* vin nv21/nv21 use */
};

COMMON_SYS_POOL_CFG_T gtPrivatePoolDoubleOs04a10Bt656Sdr[] = {
    {2688, 1520, 2688, AX_FORMAT_BAYER_RAW_10BPP_PACKED, 15, AX_COMPRESS_MODE_LOSSY, 4},      /* vin raw10 use */
};
...
COMMON_SYS_POOL_CFG_T gtPrivatePoolDoubleOs04a10Bt656Hdr[] = {
    ...
    {2688, 1520, 2688, AX_FORMAT_BAYER_RAW_10BPP_PACKED, 10, AX_COMPRESS_MODE_NONE, 0},      /* vin raw10 use */
...
COMMON_SYS_POOL_CFG_T gtSysCommPoolSingleSc850slSdr[] = {
    {3840, 2160, 3840, AX_FORMAT_YUV420_SEMIPLANAR, 16, AX_COMPRESS_MODE_LOSSY, 4},    /* vin nv21/nv21 use */
};

COMMON_SYS_POOL_CFG_T gtPrivatePoolSingleSc850slSdr[] = {
    {3840, 2160, 3840, AX_FORMAT_BAYER_RAW_10BPP_PACKED, 16, AX_COMPRESS_MODE_LOSSY, 4},      /* vin raw10 use */
};
// ### SIPEED EDIT END ###
        ...
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    ...
    pCam->eLaneComboMode = AX_LANE_COMBO_MODE_0; /* dvp only use gpio */

    return 0;
}

static AX_U32 __sample_case_single_dvp_ir(AX_CAMERA_T *pCamList, SAMPLE_SNS_TYPE_E eSnsType,
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    AX_CAMERA_T *pCam = NULL;
    COMMON_VIN_MODE_E eSysMode = pVinParam->eSysMode;
        ...
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    ...
    pCam->eLaneComboMode = AX_LANE_COMBO_MODE_0; /* bt656|bt601 only use gpio*/
        ...
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    ...
    pCam->eLaneComboMode = AX_LANE_COMBO_MODE_3; /* bt1120 */
        ...
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    ...
    for (i = 0; i < pCommonArgs->nCamCnt; i++) {
        ...
        if (i == 0) {
            ...
            pCam->nRxDev = 1;
            if (AX_SYS_GetChipType() == AX630C_CHIP || AX_SYS_GetChipType() == AX631_CHIP) {
                pCam->nI2cAddr = 0x36;
                pCam->nI2cNode = 2;
            } else {
                pCam->nI2cAddr = 0x10;
        ...
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    ...
    for (i = 0; i < pCommonArgs->nCamCnt; i++) {
        ...
        if(eSnsType == SAMPLE_SNS_BT656){
            pCam->tDevAttr.nConvYuv422To420En = 0;
            pCam->tDevAttr.nConvFactor = 0;
            pCam->tPipeAttr[pCam->nPipeId].ePipeWorkMode = AX_VIN_PIPE_SUB_YUV_MODE;
            pCam->tPipeAttr[pCam->nPipeId].ePixelFmt = AX_FORMAT_YUV422_SEMIPLANAR;
        }
        ...
        if (pCam->nPipeId == 1 && eHdrMode == AX_SNS_HDR_2X_MODE) {
            pCam->tPipeAttr[pCam->nPipeId].tCompressInfo.enCompressMode = AX_COMPRESS_MODE_NONE;
        }
        __set_pipe_hdr_mode(&pCam->tDevBindPipe.nHDRSel[0], eHdrMode);
        __set_vin_attr(pCam, eSnsType, eHdrMode, eSysMode, pVinParam->bAiispEnable);
    ...
    pCam->eLaneComboMode = AX_LANE_COMBO_MODE_1; /* bt656|bt601 only use gpio*/
                                ...
                                COMMON_SYS_ARGS_T *pPrivArgs)
{
    ...
    case SYS_CASE_SINGLE_DVP:
        ...
        break;
    case SYS_CASE_SINGLE_DVP_IR:
        eSnsType = SAMPLE_SNS_DVP_IR;
        /* comm pool config */
        __cal_dump_pool(gtSysCommPoolSingleDVPIRSdr, pVinParam->eHdrMode, pVinParam->nDumpFrameNum);
        pCommonArgs->nPoolCfgCnt = sizeof(gtSysCommPoolSingleDVPIRSdr) / sizeof(gtSysCommPoolSingleDVPIRSdr[0]);
        pCommonArgs->pPoolCfg = gtSysCommPoolSingleDVPIRSdr;

        /* private pool config */
        __cal_dump_pool(gtSysPrivatePoolSingleDVPIRSdr, pVinParam->eHdrMode, pVinParam->nDumpFrameNum);
        pPrivArgs->nPoolCfgCnt = sizeof(gtSysPrivatePoolSingleDVPIRSdr) / sizeof(gtSysPrivatePoolSingleDVPIRSdr[0]);
        pPrivArgs->pPoolCfg = gtSysPrivatePoolSingleDVPIRSdr;

        /* cams config */
        __sample_case_single_dvp_ir(pCamList, eSnsType, pVinParam, pCommonArgs);
        break;
...
AX_VOID PrintHelp()
{
    ...
    COMM_ISP_PRT("\t\t9: Triple OS04A10\n");
    COMM_ISP_PRT("\t\t10:Triple OS04A10 + SC200AIx2\n");
// ### SIPEED EDIT ###
    COMM_ISP_PRT("\t\t11:Single SC850SL\n");
// ### SIPEED EDIT END ###
...
int main(int argc, char *argv[])
{
    ...
    } else if (tVinParam.eSysCase == SAMPLE_VIN_TRIPLE_OS04A10 ||
        tVinParam.eSysCase == SAMPLE_VIN_TRIPLE_SC200AI) {
        SAMPLE_CASE_TRIPLE_SENSOR(&tVinParam);
    } else {
