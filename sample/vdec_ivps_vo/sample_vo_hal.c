...
AX_S32 SAMPLE_VoInit(SAMPLE_VO_INIT_PARAM_S *pInitParam, AX_U16 nChnNum)
{
    ...
    if (pInitParam->graphicOpen) {
        pstGraphicConf = &pstVoConf->stGraphicLayer[0];
        pstGraphicConf->u32FbNum = 1;

        pstGraphicConf->bindVoDev = pInitParam->voDev;
        pstGraphicConf->stFbConf[0].u32Index = 0;
        pstGraphicConf->stFbConf[0].u32ResoW = nWidth;
        pstGraphicConf->stFbConf[0].u32ResoH = nHeight;
        pstVoConf->stVoDev[pInitParam->voDev].u32SyncIndex = 2;

        pstGraphicConf->stFbConf[0].u32Fmt = AX_FORMAT_ARGB8888;
        pstGraphicConf->stFbConf[0].u32ColorKeyEn = 1;
        s32Ret = SAMPLE_VO_FB_CONF(1, &pstGraphicConf->stFbConf[0]);
        if (s32Ret) {
