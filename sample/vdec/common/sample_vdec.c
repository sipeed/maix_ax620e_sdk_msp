...
static AX_S32 __VdecCreateUserPool(SAMPLE_VDEC_ARGS_T *pstFuncArgs)
{
    ...
    poolWidth = pstVdGrpAttr->u32PicWidth;
    poolHeight = pstVdGrpAttr->u32PicHeight;

    if (0 == poolWidth) {
        poolWidth = pstVdGrpAttr->enCodecType == PT_H264? AX_VDEC_MAX_WIDTH : AX_JDEC_MAX_WIDTH;
    } else if (poolWidth & 0xf) {
        poolWidth = (poolWidth + 0xf)&(~0xf);
    }

    if (0 == poolHeight) {
        poolHeight = pstVdGrpAttr->enCodecType == PT_H264? AX_VDEC_MAX_HEIGHT : AX_JDEC_MAX_HEIGHT;
    } else if (poolHeight & 0xf) {
        poolHeight = (poolHeight + 0xf)&(~0xf);
    }

    u32FrameStride = AX_COMM_ALIGN(poolWidth * uPixBits, AX_VDEC_WIDTH_ALIGN * 8) / 8;

    if ((pstVdGrpAttr->enCodecType == PT_JPEG) || (pstVdGrpAttr->enCodecType == PT_MJPEG)) {
        FrameSize = AX_JDEC_GetYuvBufferSize(u32FrameStride, poolHeight, AX_FORMAT_YUV444_SEMIPLANAR);
    } else {
        FrameSize = AX_VDEC_GetPicBufferSize(u32FrameStride, poolHeight, pstVdGrpAttr->enCodecType);
    }
...
static AX_S32 __VdecGrpCreate(AX_VDEC_GRP *pVdGrp, AX_VDEC_GRP_ATTR_T *pstVdGrpAttr,
                              SAMPLE_VDEC_CMD_PARAM_T *pstCmd, AX_U32 uStreamBufSize)
{
    AX_S32 s32Ret = AX_SUCCESS;
    AX_VDEC_GRP VdGrp = *pVdGrp;
    ...
    if (pstCmd->bCreatGrpEx) {
        s32Ret = AX_VDEC_CreateGrpEx(pVdGrp, pstVdGrpAttr);
        if (s32Ret != AX_SUCCESS) {
        ...
        modInfo.enModId = AX_ID_VDEC;
        modInfo.s32GrpId = VdGrp;
        modInfo.s32ChnId = 0;
        /* set new partition name */
        s32Ret = AX_SYS_MemSetConfig(&modInfo, (AX_S8 *)"VDEC");
        if (s32Ret) {
            SAMPLE_LOG_TMP("AX_SYS_MemSetConfig fail,ret=0x%x\n", s32Ret);
        }
...
static void *_VdecGroupThreadMain(void *arg)
{
    ...
    if (pstCmd->enDecType == PT_H264) {
        ...
        SAMPLE_H264_SPS_DATA_T sps_data_new;
        sRet = h264_parse_sps(pstStreamMem, parse_len, &sps_data_new); /* inc crop info */
        SAMPLE_LOG_TMP("crop info sps_data.height:%d, sps_data.width:%d\n",
                   sps_data_new.height, sps_data_new.width);

        if (sRet == AX_SUCCESS) {
            if (pstCmd->u32PicWidth < sps_data_new.width) {
                pstCmd->u32PicWidth = sps_data_new.width;
            }

            if (pstCmd->u32PicHeight < sps_data_new.height) {
                pstCmd->u32PicHeight = sps_data_new.height;
            }
        }
    }
...
static int _VdecTestMain(SAMPLE_VDEC_CONTEXT_T *pstVdecCtx)
{
    ...
    for (i = pstCmd->uStartGrpId; i < pstCmd->uStartGrpId + pstCmd->uGrpCount; i++) {
        ...
        VdGrp = pstFuncArgs->VdGrp;
        s32Ret = __VdecGrpCreate(&VdGrp, &pstFuncArgs->tVdGrpAttr, pstCmd, 0x180000);
        if (s32Ret != AX_SUCCESS) {
    ...
    for (i = pstCmd->uStartGrpId; i < pstCmd->uStartGrpId + pstCmd->uGrpCount; i++) {
        ...
        ret = pthread_create(&pstVdecCtx->GrpTid[i], NULL, _VdecGroupThreadMain,
                             (void *)pstFuncArgs);
        if (ret) {
            SAMPLE_CRIT_LOG("VdGrp=%d, pthread_create FAILED! ret:%d, %s, \n",
                            pstFuncArgs->VdGrp, ret, strerror(ret));
            s32Ret = AX_ERR_VDEC_RUN_ERROR;
            goto ERR_RET_CANCEL_PTHREAD_RECV;
        }
        SAMPLE_LOG("VdGrp=%d, pthread_create _VdecGroupThread done! pthread_id:0x%lx \n",
                   pstFuncArgs->VdGrp, pstVdecCtx->GrpTid[i]);
    }
    ...
    for (i = pstCmd->uStartGrpId; i < pstCmd->uStartGrpId + pstCmd->uGrpCount; i++) {
        ...
        SAMPLE_LOG("VdGrp=%d, before pthread_join _VdecGroupThread pthread_id:0x%lx, PID:%d\n",
                   VdGrp, pstVdecCtx->GrpTid[i], pstVdecCtx->GrpPID[i]);
        ...
        SAMPLE_LOG("VdGrp=%d, pthread_join _VdecGroupThread done! pthread_id:0x%lx, PID:%d \n",
                   pstFuncArgs->VdGrp, pstVdecCtx->GrpTid[i], pstVdecCtx->GrpPID[i]);
    ...
    if (pstVdecCtx->tCmdParam.bSelectMode == AX_FALSE) {
        for (int gi = pstCmd->uStartGrpId; gi < pstCmd->uStartGrpId + pstCmd->uGrpCount; gi++) {
            pstFuncArgs = &pstVdecCtx->stVdecGrpArgs[gi];
            VdGrp = pstFuncArgs->VdGrp;
            SAMPLE_LOG("VdGrp=%d, before pthread_join pthread_id:0x%lx",
                        VdGrp, pstVdecCtx->GrpChnRecvTid[gi]);

            ret = pthread_join(pstVdecCtx->GrpChnRecvTid[gi], NULL);
            if (ret) {
...
ERR_RET_CANCEL_PTHREAD_RECV:
    if (pstVdecCtx->tCmdParam.bSelectMode == AX_FALSE) {
        for (int gi = pstCmd->uStartGrpId; gi < pstCmd->uStartGrpId + pstCmd->uGrpCount; gi++) {
            pstFuncArgs = &pstVdecCtx->stVdecGrpArgs[gi];
            VdGrp = pstFuncArgs->VdGrp;
            ret = pthread_cancel(pstVdecCtx->GrpChnRecvTid[gi]);
            if (ret) {
...
int Sample_VdecJpegDecodeOneFrame(SAMPLE_VDEC_CMD_PARAM_T *pstCmd)
{
    ...
    if ((pstCmd->enDecType == PT_JPEG) || (pstCmd->enDecType == PT_MJPEG)) {
        /* alloc yuv size = widthAlign * heightAlign * 3 */
        uBufSize = AX_JDEC_GetYuvBufferSize(frmStride, heightAlign, AX_FORMAT_YUV444_SEMIPLANAR);
    } else {
