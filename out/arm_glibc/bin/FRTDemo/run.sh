...
if [ $# -lt 2 ] ; then
  ...
  echo "   1000: OS04A10+OS04A10+OS04A10 (Triple sensor)"
  echo "   2033: OS04A10+SC200AIx2 (Dual sensor show, OS04A10+SC200AIx2 switch with single pipe)"
  echo "   2500: SC850SL+OS04A10x2 (Dual sensor show, SC850SL+OS04A10x2 switch with single pipe)"
  ...
  echo "      6: Triple sensor"
  echo "      7: Triple sensor(Dual sensor show, switch with single pipe)"
...
while getopts "p:s:n:l:t:c:ad" opt
do
   case "$opt" in
      p ) p_value="$OPTARG" ;;
      n ) n_value="$OPTARG" ;;
...
if [ $p_value == 0 -a $n_value == 7 ] ; then
    ax_lookat 0x0230406C -s 0x00040083
    ax_lookat 0x02304084 -s 0x00040083
    ax_lookat 0x02302048 -s 0x00010083
fi
