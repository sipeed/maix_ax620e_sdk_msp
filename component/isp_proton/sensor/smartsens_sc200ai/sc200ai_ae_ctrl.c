...
AX_U8 gSc200aiSlaveMode[AX_VIN_MAX_PIPE_NUM] = {0};

/*user config*/
static AX_F32 gFpsGear[] = {1.00, 2.00, 3.00, 4.00, 5.00, 6.00, 7.00, 8.00, 9.00, 10.00,
                            11.00, 12.00, 13.00, 14.00, 15.00, 16.00, 17.00, 18.00, 19.00, 20.00,
                            21.00, 22.00, 23.00, 24.00, 25.00, 26.00, 27.00, 28.00, 29.00, 30.00, 60
                           };

static AX_SNS_DRV_DELAY_TABLE_T gSc200aiAeRegsTable_Master[] = {
...
static AX_SNS_DRV_DELAY_TABLE_T gSc200aiAeRegsTable_slave[] = {
    /* regs index */       /* regs addr */        /*regs value*/  /*Delay Frame Num*/
    {SC200AI_EXPLINE_LONG_H_IDX,    SC200AI_EXPLINE_LONG_H,     0,      0},
    {SC200AI_EXPLINE_LONG_M_IDX,    SC200AI_EXPLINE_LONG_M,     0,      0},
    {SC200AI_EXPLINE_LONG_L_IDX,    SC200AI_EXPLINE_LONG_L,     0,      0},
...
AX_S32 sc200ai_get_params_from_setting(ISP_PIPE_ID nPipeId)
{
    ...
    ret = sc200ai_get_exp_offset_from_setting(nPipeId, setting, reg_cnt, &exp_offset);
    if(ret) {
        return ret;
    }

    gSc200aiParams[nPipeId].vts = vts;
    gSc200aiParams[nPipeId].exp_offset = exp_offset;

    SNS_DBG("pipe:%d, get setting params vts:0x%x exp_offset = %.6f\n",
        nPipeId, gSc200aiParams[nPipeId].vts, gSc200aiParams[nPipeId].exp_offset);
...
AX_S32 sc200ai_cfg_aec_param(ISP_PIPE_ID nPipeId)
{
    ...
    if (sns_obj->ae_ctrl_param.fTimePerLine) {
        return AX_SNS_SUCCESS;
    }
    ...
    if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_LINEAR_MODE) {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = gSc200aiParams[nPipeId].exp_offset;
    } else if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_HDR_2X_MODE) {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = gSc200aiParams[nPipeId].exp_offset;
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_MEDIUM_FRAME_IDX] = gSc200aiParams[nPipeId].exp_offset;
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = gSc200aiParams[nPipeId].exp_offset;
    }
...
AX_S32 sc200ai_set_again(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptAGain)
{
    ...
    if (result) {
        ...
        sc200ai_sns_update_regs_table(nPipeId, SC200AI_AGAIN_LONG_H, gain_in);
        sc200ai_sns_update_regs_table(nPipeId, SC200AI_AGAIN_LONG_L, gain_de);
    ...
    if (IS_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        if (result) {
            ...
            sc200ai_sns_update_regs_table(nPipeId, SC200AI_AGAIN_SHORT_H, gain_in);
            sc200ai_sns_update_regs_table(nPipeId, SC200AI_AGAIN_SHORT_L, gain_de);
...
AX_S32 sc200ai_set_dgain(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptDGain)
{
    ...
    if (result) {
        ...
        sc200ai_sns_update_regs_table(nPipeId, SC200AI_DGAIN_LONG_H, gain_in);
        sc200ai_sns_update_regs_table(nPipeId, SC200AI_DGAIN_LONG_L, gain_de);
    ...
    if (IS_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        if (result) {
            ...
            sc200ai_sns_update_regs_table(nPipeId, SC200AI_DGAIN_SHORT_H, gain_in);
            sc200ai_sns_update_regs_table(nPipeId, SC200AI_DGAIN_SHORT_L, gain_de);
...
AX_S32 sc200ai_set_integration_time(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    ...
    /* Workround for AOV, drop the first frame after wake up. */
    if (sns_obj->sns_mode_obj.eMasterSlaveSel == AX_SNS_SYNC_SLAVE) {
        sns_obj->sztRegsInfo[0].nDropFrame = 1;
    } else {
        sns_obj->sztRegsInfo[0].nDropFrame = (exp_reg <= 30) ? 1 : 0;
    }

    sc200ai_sns_update_regs_table(nPipeId, SC200AI_EXPLINE_LONG_H, exp_h);
    sc200ai_sns_update_regs_table(nPipeId, SC200AI_EXPLINE_LONG_M, exp_m);
    sc200ai_sns_update_regs_table(nPipeId, SC200AI_EXPLINE_LONG_L, exp_l);
    ...
    if (IS_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        sc200ai_sns_update_regs_table(nPipeId, SC200AI_EXPLINE_SHORT_H, exp_h);
        sc200ai_sns_update_regs_table(nPipeId, SC200AI_EXPLINE_SHORT_M, exp_m);
        sc200ai_sns_update_regs_table(nPipeId, SC200AI_EXPLINE_SHORT_L, exp_l);
...
AX_U32 sc200ai_refresh_all_regs_from_tbl(ISP_PIPE_ID nPipeId)
{
    AX_S32 i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    AX_SNS_DRV_DELAY_TABLE_T *gSc200aiAeRegsTable = AX_NULL;

    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
    SENSOR_GET_CTX(nPipeId, sns_obj);

    if (sns_obj->sns_mode_obj.eMasterSlaveSel == AX_SNS_SYNC_SLAVE) {
        gSc200aiAeRegsTable = gSc200aiAeRegsTable_slave;
        sns_obj->sztRegsInfo[0].nRegNum = sizeof(gSc200aiAeRegsTable_slave) / sizeof(gSc200aiAeRegsTable_slave[0]);
    } else {
        gSc200aiAeRegsTable = gSc200aiAeRegsTable_Master;
        sns_obj->sztRegsInfo[0].nRegNum = sizeof(gSc200aiAeRegsTable_Master) / sizeof(gSc200aiAeRegsTable_Master[0]);
    }

    SNS_CHECK_PTR_VALID(gSc200aiAeRegsTable);
    ...
    for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
        ...
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nData = sc200ai_reg_read(nPipeId, sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nRegAddr);
#endif
...
AX_S32 sc200ai_set_fps(ISP_PIPE_ID nPipeId, AX_F32 fFps)
{
    ...
    if (IS_SNS_FPS_EQUAL(fFps, gSc200aiParams[nPipeId].setting_fps)) {
        ...
        if (vts > SC200AI_MAX_VTS){
            vts = SC200AI_MAX_VTS;
            SNS_WRN("Beyond max vts:0x%x, fTimePerLine:%f, fFps:%f\n", vts, sns_obj->ae_ctrl_param.fTimePerLine, fFps);
        }
    }

    for (i = 0; i < AX_VIN_MAX_PIPE_NUM; i++) {
        if(gSc200aiSlaveMode[i] == AX_TRUE) {
            tVsyncInfo.tI2cInfo[j].nI2cNode = sc200ai_get_bus_num(i);
...
AX_S32 sc200ai_get_slow_shutter_param(ISP_PIPE_ID nPipeId, AX_SNS_AE_SLOW_SHUTTER_PARAM_T *ptSlowShutterParam)
{
    ...
    for (i = 0; i < ptSlowShutterParam->nGroupNum; i++) {
        ...
        if ((AX_S32)gFpsGear[i] > nFrameRate) {
            vts = 1 * SNS_1_SECOND_UNIT_US / (sns_obj->ae_ctrl_param.fTimePerLine * nFrameRate);
        }
        if (vts > SC200AI_MAX_VTS) {
            vts = SC200AI_MAX_VTS;
