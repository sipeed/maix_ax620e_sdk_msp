...
AX_S32 sc500ai_again2value(AX_F32 gain, AX_U8 *again_in, AX_U8 *again_de, AX_F32 *gain_value)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    for (i = 0; i < count; i++) {
        if (gain > sc500ai_again_table[i].gain) {
            ...
            *gain_value = sc500ai_again_table[i].gain;
...
AX_S32 sc500ai_dgain2value(AX_F32 gain, AX_U8 *dgain_in, AX_U8 *dgain_de, AX_F32 *gain_value)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    for (i = 0; i < count; i++) {
        if (gain > sc500ai_dgain_table[i].gain) {
            ...
            *gain_value = sc500ai_dgain_table[i].gain;
...
AX_S32 sc500ai_get_params_from_setting(ISP_PIPE_ID nPipeId)
{
    ...
    ret = sc500ai_get_exp_offset_from_setting(nPipeId, setting, reg_cnt, &exp_offset);
    if (ret) {
        return ret;
    }

    gSc500aiParams[nPipeId].vts = vts;
    gSc500aiParams[nPipeId].exp_offset = exp_offset;

    SNS_DBG("pipe:%d, get setting params vts:0x%x exp_offset = %.6f\n",
        nPipeId, gSc500aiParams[nPipeId].vts, gSc500aiParams[nPipeId].exp_offset);
...
AX_S32 sc500ai_cfg_aec_param(ISP_PIPE_ID nPipeId)
{
    ...
    gSc500aiParams[nPipeId].setting_vts = gSc500aiParams[nPipeId].vts;
    gSc500aiParams[nPipeId].setting_fps = sns_obj->sns_mode_obj.fFrameRate;

    /* update initial vts to reg table */
    sc500ai_set_vts(nPipeId, sns_obj->sns_mode_obj.nVts);
    ...
    if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_LINEAR_MODE) {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = gSc500aiParams[nPipeId].exp_offset;
    } else if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_HDR_2X_MODE) {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = gSc500aiParams[nPipeId].exp_offset;
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_MEDIUM_FRAME_IDX] = gSc500aiParams[nPipeId].exp_offset;
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = gSc500aiParams[nPipeId].exp_offset;
    }
...
AX_S32 sc500ai_set_again(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptAGain)
{
    ...
    result = sc500ai_again2value(fGainFromUser, &gain_in, &gain_de, &gain_value);
    if (result) {
...
AX_S32 sc500ai_set_dgain(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptDGain)
{
    ...
    result = sc500ai_dgain2value(fGainFromUser, &gain_in, &gain_de, &gain_value);
    if (result) {
        SNS_ERR("new gain match failed \n");
        return result;
    } else {
...
AX_S32 sc500ai_set_integration_time(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    ...
    if (IS_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        //LineGap < Height
        LineGap = sc500ai_get_vts_s(nPipeId) / 2;
    ...
    SNS_INFO("userExpline:%f-%f, userRatio:%.2f, curExpline:%f-%f(%f-%f), curRatio:%.2f, line_period:%f\n",
...
AX_S32 sc500ai_set_fps(ISP_PIPE_ID nPipeId, AX_F32 fFps)
{
    ...
    sc500ai_set_exp_limit(nPipeId, SC500AI_MAX_RATIO, vts);
    sns_obj->ae_ctrl_param.sns_ae_param.fCurFps = SNS_1_SECOND_UNIT_US / (sns_obj->ae_ctrl_param.fTimePerLine * vts);
...
AX_S32 sc500ai_get_slow_shutter_param(ISP_PIPE_ID nPipeId, AX_SNS_AE_SLOW_SHUTTER_PARAM_T *ptSlowShutterParam)
{
    ...
    for (i = 0; i < ptSlowShutterParam->nGroupNum; i++) {
        ...
        SNS_INFO("pipe=%d, ratio=%.2f, fps=%f, max_expline=%f(%f), vts=0x%x, vts_s=0x%x, line_period=%f\n",
