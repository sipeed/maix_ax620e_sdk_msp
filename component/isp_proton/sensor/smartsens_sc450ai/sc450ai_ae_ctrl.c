...
AX_S32 sc450ai_get_params_from_setting(ISP_PIPE_ID nPipeId)
{
    ...
    ret = sc450ai_get_exp_offset_from_setting(nPipeId, setting, reg_cnt, &exp_offset);
    if (ret) {
        return ret;
    }

    sns_sc450aiparams[nPipeId].vts = vts;
    sns_sc450aiparams[nPipeId].exp_offset = exp_offset;

    SNS_DBG("pipe:%d, get setting params vts:0x%x exp_offset = %.6f\n",
        nPipeId, sns_sc450aiparams[nPipeId].vts, sns_sc450aiparams[nPipeId].exp_offset);
...
AX_S32 sc450ai_cfg_aec_param(ISP_PIPE_ID nPipeId)
{
    ...
    if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_LINEAR_MODE) {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = sns_sc450aiparams[nPipeId].exp_offset;
    } else if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_HDR_2X_MODE) {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = sns_sc450aiparams[nPipeId].exp_offset;
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_MEDIUM_FRAME_IDX] = sns_sc450aiparams[nPipeId].exp_offset;
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = sns_sc450aiparams[nPipeId].exp_offset;
    }
