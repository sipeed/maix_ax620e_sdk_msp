...
#include "sc450ai_sdr_qs.h"
#include "sc450ai_hdr_2x_qs.h"
#else
...
static void sc450ai_init(ISP_PIPE_ID nPipeId)
{
...
#else
    sns_obj->sns_id = SC450AI_SENSOR_ID;
#endif
    /* 4. refresh ae param */
    ...
    sc450ai_get_sensor_stream_ctrl(nPipeId);
...
AX_S32 sc450ai_sleep_wakeup(ISP_PIPE_ID nPipeId, AX_SNS_SLEEP_WAKEUP_E eSleepWakeup)
{
    AX_S32 result = 0;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);

    if (AX_SNS_EVENT_WAKE_UP == eSleepWakeup) {
        sc450ai_hw_reset(35, 1);
        result = sc450ai_write_register(nPipeId, 0x0100, 0x01); // wakeup
        SNS_DBG("sensor wakeup !\n");
    } else if (AX_SNS_EVENT_SLEEP == eSleepWakeup) {
        result = sc450ai_write_register(nPipeId, 0x0100, 0x00); // sleep
        sc450ai_hw_reset(35, 0);
...
static AX_S32 sc450ai_get_3a_default_params(ISP_PIPE_ID nPipeId, AX_SENSOR_3A_DEFAULT_PARAM_T *ptDftParam)
{
#ifdef USE_DEFAULT_PARAM
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_SYS_API_PUBLIC AX_SENSOR_REGISTER_FUNC_T gSnssc450aiObj = {
    ...
    .pfn_sensor_sleep_wakeup                = sc450ai_sleep_wakeup,
    ...
    /* default param */
    .pfn_sensor_get_isp_default_params      = sc450ai_get_isp_default_params,
    .pfn_sensor_get_3a_default_params       = sc450ai_get_3a_default_params,
