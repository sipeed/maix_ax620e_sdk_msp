...
static const OS04A10_AGAIN_TABLE_T os04a10_dcg_again_table[] = {
    {1,       0x01, 0x00},
    {1.0625,  0x01, 0x10},
    {1.125,   0x01, 0x20},
    {1.1875,  0x01, 0x30},
...
static const OS04A10_DGAIN_TABLE_T os04a10_dcg_dgain_table[] = {
...
static AX_S32 os04a10_again2value_dcg(AX_F32 gain, AX_U8 *again_in, AX_U8 *again_de, AX_F32 *gain_value)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    for (i = 0; i < count; i++) {
        if (gain > os04a10_dcg_again_table[i].gain) {
            ...
            *gain_value = os04a10_dcg_again_table[i].gain;
...
static AX_S32 os04a10_dgain2value_dcg(float gain, AX_U8 *dgain_in, AX_U8 *dgain_de, AX_U8 *dgain_de2, AX_F32 *gain_value)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    for (i = 0; i < count; i++) {
        if (gain > os04a10_dcg_dgain_table[i].gain) {
            ...
            *gain_value = os04a10_dcg_dgain_table[i].gain;
...
static AX_S32 os04a10_get_gain_table_dcg(AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_U32 i;
    AX_S32 ret = 0;
    if (!params)
...
AX_S32 os04a10_get_params_from_setting_dcg(ISP_PIPE_ID nPipeId)
{
    AX_S32 ret = 0;
    AX_U32 reg_cnt = 0;
...
AX_S32 os04a10_cfg_aec_param_dcg(ISP_PIPE_ID nPipeId)
{
    ...
    os04a10_get_params_from_setting_dcg(nPipeId);
...
static AX_S32 os04a10_sns_update_regidx_table_dcg(ISP_PIPE_ID nPipeId, AX_U8 nRegIdx, AX_U8 nRegValue)
{
    AX_S32 i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
AX_S32 os04a10_set_again_dcg(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptAGain)
{
    ...
    result = os04a10_again2value_dcg(nGainFromUser, &Gain_in, &Gain_de, &gain_value);
    if (result) {
...
AX_S32 os04a10_set_dgain_dcg(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptDGain)
{
    ...
    result = os04a10_dgain2value_dcg(nGainFromUser, &Gain_in, &Gain_de, &Gain_de2, &gain_value);
    if (result) {
        SNS_ERR("new gain match failed \n");
        return result;
    } else {
...
static AX_S32 os04a10_sns_update_init_exposure_reg_dcg(ISP_PIPE_ID nPipeId)
{
    int i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
 ...
 ****************************************************************************/
static AX_S32 os04a10_ctx_init_dcg(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    AX_S32 ret = 0;

    SNS_DBG("os04a10 os04a10_ctx_init_dcg. ret = %d\n", ret);
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
static AX_VOID os04a10_ctx_exit_dcg(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static void os04a10_init_dcg(ISP_PIPE_ID nPipeId)
{
    ...
    if (AX_NULL == sns_obj) {
        /* contex init */
        nRet = os04a10_ctx_init_dcg(nPipeId);
        if (0 != nRet) {
            SNS_ERR("os04a10_ctx_init_dcg failed!\n");
            return;
        } else {
            SENSOR_GET_CTX(nPipeId, sns_obj);
        }
 ...
 ****************************************************************************/
static AX_S32 os04a10_get_3a_default_params_dcg(ISP_PIPE_ID nPipeId, AX_SENSOR_3A_DEFAULT_PARAM_T *ptDftParam)
{
#ifdef USE_DEFAULT_PARAM
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_SYS_API_PUBLIC AX_SENSOR_REGISTER_FUNC_T gSnsos04a10ObjDcg = {
    ...
    /* default param */
    .pfn_sensor_get_isp_default_params      = os04a10_get_isp_default_params_dcg,
    .pfn_sensor_get_3a_default_params       = os04a10_get_3a_default_params_dcg,
