...
AX_S32 os04a10_get_integration_time_range_sf(ISP_PIPE_ID nPipeId, AX_F32 fHdrRatio,
...
AX_S32 os04a10_set_integration_time_sf(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    ...
    SNS_DBG("[SF] Exp line: long frame %d, short frame %d\n",
            sns_obj->sztRegsInfo[0].tSnsExpInfo.szExpTime[HDR_LONG_FRAME_IDX],
            sns_obj->sztRegsInfo[0].tSnsExpInfo.szExpTime[HDR_MEDIUM_FRAME_IDX]);
...
AX_S32 os04a10_get_hw_exposure_params_sf(ISP_PIPE_ID nPipeId, AX_SNS_EXP_CTRL_PARAM_T *ptAeCtrlParam)
{
    ...
    ptAeCtrlParam->sns_dev_attr.eSnsMode = AX_SNS_LINEAR_MODE; //long/short frame mode ae run in sdr mod
...
AX_S32 os04a10_set_slow_fps_sf(ISP_PIPE_ID nPipeId, AX_F32 nFps)
{
    AX_S32 result = 0;
    AX_S32 framerate = 30;
    AX_U32 vts = 0;
