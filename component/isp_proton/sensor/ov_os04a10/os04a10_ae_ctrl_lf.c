...
static const OS04A10_AGAIN_TABLE_T os04a10_again_table[] = {
    {1,       0x01, 0x00},
    {1.0625,  0x01, 0x10},
    {1.125,   0x01, 0x20},
    {1.1875,  0x01, 0x30},
...
static const OS04A10_DGAIN_TABLE_T os04a10_dgain_table[] = {
...
static AX_S32 os04a10_again2value(AX_F32 gain, AX_U8 *again_in, AX_U8 *again_de, AX_F32 *gain_value)
{
    AX_U32 i;
    AX_U32 count;
...
static AX_S32 os04a10_dgain2value(AX_F32 gain, AX_U8 *dgain_in, AX_U8 *dgain_de, AX_U8 *dgain_de2, AX_F32 *gain_value)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
...
static AX_S32 os04a10_get_gain_table(AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_U32 i;
...
AX_S32 os04a10_cfg_aec_param_lf(ISP_PIPE_ID nPipeId)
{
    ...
    if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] =
            sns_os04a10params[nPipeId].vts - 8;
        ...
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_MEDIUM_FRAME_IDX] =
            sns_os04a10params[nPipeId].vts - 6;
    ...
    sns_obj->ae_ctrl_param.fSnsHcgLcgRatio = 4.22265625f;
...
static AX_S32 os04a10_sns_update_regidx_table(ISP_PIPE_ID nPipeId, AX_U8 nRegIdx, AX_U8 nRegValue)
{
    AX_S32 i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
AX_S32 os04a10_get_sensor_gain_table_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_S32 result = 0;
    SNS_CHECK_PTR_VALID(params);
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
AX_S32 os04a10_set_again_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptAGain)
{
    AX_U8 Gain_in;
    AX_U8 Gain_de;
    AX_S32 result = 0;
...
AX_S32 os04a10_set_dgain_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptDGain)
{
    AX_U8 Gain_in;
    AX_U8 Gain_de;
    AX_U8 Gain_de2;
...
AX_S32 os04a10_get_integration_time_range_lf(ISP_PIPE_ID nPipeId, AX_F32 fHdrRatio,
        AX_SNS_AE_INT_TIME_RANGE_T *ptIntTimeRange)
{
    ...
    if (AX_SNS_HDR_2X_MODE == sns_obj->sns_mode_obj.eHDRMode) {
        ...
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] =
            (sns_os04a10params[nPipeId].vts - 4) * ((ratio) / (1 + ratio));

        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_MEDIUM_FRAME_IDX] =
            (sns_os04a10params[nPipeId].vts - 4) -
            sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX];
...
#else
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] =
            (sns_os04a10params[nPipeId].vts - 8) / 2;
...
AX_S32 os04a10_set_integration_time_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    AX_U8 ex_h;
    AX_U8 ex_l;
    AX_U32 ex_ival = 0;
    ...
    SNS_DBG("[LF] Exp line: long frame %d, short frame %d\n",
            sns_obj->sztRegsInfo[0].tSnsExpInfo.szExpTime[HDR_LONG_FRAME_IDX],
            sns_obj->sztRegsInfo[0].tSnsExpInfo.szExpTime[HDR_MEDIUM_FRAME_IDX]);
...
AX_S32 os04a10_get_hw_exposure_params_lf(ISP_PIPE_ID nPipeId, AX_SNS_EXP_CTRL_PARAM_T *ptAeCtrlParam)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    ptAeCtrlParam->sns_dev_attr.eSnsMode = AX_SNS_LINEAR_MODE; //long/short frame mode ae run in sdr mode
...
AX_S32 os04a10_hcglcg_ctrl_lf(ISP_PIPE_ID nPipeId, AX_U32 nSnsHcgLcg)
{
...
AX_S32 os04a10_sns_refresh_all_regs_from_tbl_lf(ISP_PIPE_ID nPipeId)
{
    AX_S32 i = 0;
    AX_U32 nNum = 0;
    AX_U32  nRegValue;
...
static AX_S32 os04a10_sns_update_init_exposure_reg(ISP_PIPE_ID nPipeId)
{
    int i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
AX_S32 os04a10_ae_get_sensor_reg_info_lf(ISP_PIPE_ID nPipeId, AX_SNS_REGS_CFG_TABLE_T *ptSnsRegsInfo)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_S32 os04a10_set_slow_fps_lf(ISP_PIPE_ID nPipeId, AX_F32 nFps)
{
    AX_S32 result = 0;
    AX_S32 framerate = 30;
    AX_U32 vts = 0;
    ...
    if(AXSNS_CAMPARE_FLOAT(AX_SNS_FPS_MIN, nFps) || AXSNS_CAMPARE_FLOAT(nFps, AX_SNS_FPS_MAX)) {
        SNS_ERR("pipe:%d, fps:%f is not supported, range:[%f-%f]\n", nPipeId, nFps, AX_SNS_FPS_MIN, AX_SNS_FPS_MAX);
    ...
    sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = (sns_os04a10params[nPipeId].vts - 8);
    sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_MEDIUM_FRAME_IDX] = (sns_os04a10params[nPipeId].vts - 8);
...
AX_S32 os04a10_get_slow_shutter_param_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_SLOW_SHUTTER_PARAM_T *ptSlowShutterParam)
{
     AX_S32 framerate = 30;
    AX_U32 nfps = 0;
    AX_U32 nVts = 0;
    ...
    for (nfps = 0 ; nfps < ptSlowShutterParam->nGroupNum; nfps++) {
        ...
        if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
            ...
            ptSlowShutterParam->tSlowShutterTbl[nfps].fMaxIntTime = (AX_U32)((nVts  - 8)
...
AX_S32 os04a10_get_fps_lf(ISP_PIPE_ID nPipeId, AX_F32 *pFps)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_S32 os04a10_set_fps_lf(ISP_PIPE_ID nPipeId, AX_F32 fFps)
{
    AX_S32 result = 0;
    AX_U32 vts = 0;
    ...
    if(IS_SNS_FPS_EQUAL(fFps, 25.0) || IS_SNS_FPS_EQUAL(fFps, 50.0)) {  /* PAL */
        if(AX_SNS_LINEAR_MODE == sns_obj->sns_attr_param.eSnsMode) {
            if(AX_RT_RAW10 == sns_obj->sns_attr_param.eRawType) {
                ...
                if (sns_obj->sns_attr_param.nSettingIndex > 3) {
