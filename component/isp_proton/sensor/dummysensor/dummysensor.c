 ...
 ****************************************************************************/
static AX_S32 dummysensor_ctx_init(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    AX_S32 ret = 0;

    SNS_DBG("dummysensor dummysensor_ctx_init. ret = %d\n", ret);
...
static AX_VOID dummysensor_ctx_exit(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static void dummysensor_init(ISP_PIPE_ID nPipeId)
{
    ...
    if (AX_NULL == sns_obj) {
        /* contex init */
        nRet = dummysensor_ctx_init(nPipeId);
        if (0 != nRet) {
            SNS_ERR("dummysensor_ctx_init failed!\n");
            return;
        } else {
            SENSOR_GET_CTX(nPipeId, sns_obj);
        }
 ...
 ****************************************************************************/
static AX_S32 dummy_get_3a_default_params(ISP_PIPE_ID nPipeId, AX_SENSOR_3A_DEFAULT_PARAM_T *ptDftParam)
{
#ifdef USE_DEFAULT_PARAM
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_SYS_API_PUBLIC AX_SENSOR_REGISTER_FUNC_T gSnsdummyObj = {
    ...
    /* default param */
    .pfn_sensor_get_isp_default_params      = dummy_get_isp_default_params,
    .pfn_sensor_get_3a_default_params       = dummy_get_3a_default_params,
